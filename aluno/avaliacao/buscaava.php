<?php
  ...
  if(isset($_POST['busca'])){
    if($_POST['busca'] == "unica"){
      $idaluno = $_POST['idaluno'];

      $sql = "SELECT tbaluno.nome AS aluno, tbeducador.nome AS educador, MAX(tbavaliacao.datava) AS dataava, tbavaliacao.horaava AS horaava, tbavaliacao.med_braco AS braco, tbavaliacao.med_antebraco AS antebraco, tbavaliacao.med_coxa AS coxa, tbavaliacao.med_panturrilha AS panturrilha, tbavaliacao.objetivo AS objetivo
      FROM tbavaliacao
      ...
      WHERE tbavaliacao.idaluno = ".$idaluno;
    ...
    if($_POST['busca'] == "todas"){
      $idaluno = $_POST['idaluno'];

      $sql = "SELECT tbavaliacao.idava AS idava, tbeducador.nome AS educador, tbavaliacao.datava AS dataava, tbavaliacao.horaava AS horaava
      FROM tbavaliacao
      INNER JOIN tbeducador ON(tbeducador.ideducador = tbavaliacao.ideducador)
      WHERE tbavaliacao.idaluno = ".$idaluno;
      ...
      if($consulta){
        if($consulta -> num_rows > 0){
          while($linha = $consulta ->fetch_array(MYSQLI_ASSOC)){
            echo '<tr>
                  <td>'.$linha['educador'].'</td>
                  <td>'.$linha['dataava'].'</td>
                  <td>'.$linha['horaava'].'</td>
                  <td>
                    <button class="btn btn-primary" id="verAva" value="'.$linha['idava'].'"><i class="fa-solid fa-eye"></i></button>
                  </td>
                </tr>';
          }
        }
